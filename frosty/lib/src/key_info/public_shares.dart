...
class PublicSharesKeyInfo({required PublicShareList publicShares})
    extends KeyInfo {
  /// The public shares for each participant, ordered by identifier
  final PublicShareList list = List.from(publicShares)
    ..sort((a, b) => a.$1.compareTo(b.$1));

  this {
    if (publicShares.map((share) => share.$1).toSet().length !=
        publicShares.length) {
  ...
  PublicSharesKeyInfo? tweak(Uint8List scalar) {
    final newShares = list.map((share) {
